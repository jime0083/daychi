 ...
 * 管理画面向けコンポーネント(タスク2-4: 店舗登録CRUD。タスク2-4cで緯度経度の
 * 数値入力欄を撤去し、住所ジオコーディングボタン方式に変更。problem.txt P-011対応)。
 *
 * 設計判断:
 ...
 *   renderされない前提)。
 * - 緯度経度の確定は、呼び出し側(/admin/shops)が持つ「住所からピンを立てる」
 *   ボタン(/api/admin/geocode呼び出し)、またはこのコンポーネント自身の地図
 *   クリック/ピンドラッグの2通りで行う(数値入力欄はUIに存在しない。
 *   タスク2-4cで撤去)。E2Eでの座標指定はジオコードAPIをモックして決定的に行い
 *   (e2e/support/geocode-mock.ts参照)、地図タイル自体は外部ネットワーク依存の
 *   ためブロックしてE2Eをflakyにしないようにする。
